...
const SystemRecovery: React.FC<SystemRecoveryProps> = ({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-slate-900 dark:text-slate-100">
      ...
      {isSuperAdmin && onPurgeDemoData && (
        <div className="bg-white/90 dark:bg-[linear-gradient(180deg,#132248_0%,#0b1431_100%)] p-8 rounded-3xl border border-slate-200/80 dark:border-blue-400/20 shadow-[0_10px_30px_rgba(15,23,42,0.12)] dark:shadow-[0_10px_30px_rgba(0,0,0,0.35)] space-y-4 backdrop-blur">
          <div>
            <h3 className="text-xl font-black uppercase tracking-tight">Demo Data</h3>
            <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-2">
              Purge autogenerated samples
            </p>
          </div>
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-bold text-amber-800">
            This removes autogenerated demo candidates/bookings and disables future demo seeding on this workspace.
          </div>
          <button
            onClick={onPurgeDemoData}
            className="w-full py-3 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-black uppercase tracking-widest shadow-lg"
          >
            Purge Demo Data
          </button>
        </div>
