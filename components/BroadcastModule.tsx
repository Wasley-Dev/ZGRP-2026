...
const BroadcastModule: React.FC<{ candidates: Candidate[] }> = ({ candidates }) => {
  ...
  const filteredCandidates = useMemo(() => {
    ...
    return candidates.filter((c) => target.includes(c.status));
  }, [candidates, target]);

  const resolveRecipients = () => {
    const emails = Array.from(
      new Set(filteredCandidates.map((c) => c.email).filter(Boolean))
    );
    const phones = Array.from(
      new Set(filteredCandidates.map((c) => c.phone).filter(Boolean))
    );
  ...
  const handleSend = () => {
    ...
    const { emails, phones } = resolveRecipients();
    if (channel === 'Email') {
      const bcc = emails.join(',');
      const subject = 'ZAYA GROUP Notification';
    ...
    if (channel === 'SMS') {
      ...
      if (phones.length > max) {
        alert(`SMS sent to first ${max} numbers. ${phones.length - max} remaining.`);
      }
    }
    ...
    if (channel === 'WhatsApp') {
      ...
      if (phones.length > max) {
        alert(`WhatsApp opened for first ${max} numbers. ${phones.length - max} remaining.`);
      }
    }
